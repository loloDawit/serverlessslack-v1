...
class Client {
  ...
  reply (message, ephemeral) {
    ...
    if (!this.responseUrl && ephemeral) {
      return Promise.reject(new Error("Message can't be private"));

      // slash commands and interactive messages
    }
    if (this.responseUrl) {
      if (!ephemeral) message.response_type = 'in_channel';
      return this.send(this.responseUrl, message);

      // incoming webhooks
    }
    if (this.auth.incoming_webhook && !this.channel && !message.channel) {
  ...
  send (endPoint, message) {
    ...
    message = { token: this.token, channel: this.channel, ...message };
